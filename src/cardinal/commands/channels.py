...
class Channels(Cog):
    ...
    async def _list(self, ctx: commands.Context):
        ...
        with session_scope() as session:
            channel_iter = filter(None, (discord.utils.get(ctx.guild.text_channels, id=db_channel.id) for db_channel in session.query(Channel).filter_by(guild_id=ctx.guild.id)))
