...
class Stop(BaseCog):
    ...
    async def _lock_channel(self, channel):
        ...
        await maybe_send(channel, 'Sending messages to this channel has been restricted.')

        overwrite = channel.overwrites_for(role)
        self._perm_cache.setdefault(channel.id,
    ...
    async def _unlock_channel(self, channel):
        ...
        await maybe_send(channel, 'Sending messages to this channel has been unrestricted.')
    ...
    async def _on_all(self, ctx):
        ...
        coros = (self._lock_channel(channel) for channel in ctx.guild.text_channels
    ...
    async def _off_all(self, ctx):
        ...
        """
        public_channels = {channel.id: channel for channel in ctx.guild.text_channels
                           if is_public(channel)}
        ...
        coros = (self._unlock_channel(public_channels[channel_id])
